...
public class DetayliEkran extends AppCompatActivity {
    ...
    public void bilgileri_ekle() {
        ...
        else if (urun_adi.equals("MilkaCikolataCilekliYogurtlu"))
            urun_img.setImageResource(R.drawable.milka_cilekli_yogurtlu_tablet_cikolata);

        else if (urun_adi.equals("UlkerBiskremKakaoluBiskuviBuyuk"))
            urun_img.setImageResource(R.drawable.ulker_biskrem_biskuvi_cikolata_dolgulu);

        else if (urun_adi.equals("HariboAltinAyicik"))
            urun_img.setImageResource(R.drawable.haribo_altin_ayicik);

        else if (urun_adi.equals("EtiHosbesCilek"))
            urun_img.setImageResource(R.drawable.eti_hosbes_gofret_cilekli);

        else if (urun_adi.equals("EtiBurcakKurabi"))
            urun_img.setImageResource(R.drawable.eti_burcak_kurabi);

        else if (urun_adi.equals("CalgonAntibakteriyelJel"))
            urun_img.setImageResource(R.drawable.calgon_antibakteriyel_jel_750_ml);

        else if (urun_adi.equals("CifJelTumYuzeyler"))
            urun_img.setImageResource(R.drawable.cif_jel_tum_yuzeyler_cicek_ferahligi);

        else if (urun_adi.equals("DomestosCamasirSuyuKarBeyazi"))
            urun_img.setImageResource(R.drawable.domestos_camasir_suyu_kar_beyazi);

        else if (urun_adi.equals("PersilSiviCamasirDeterjani"))
            urun_img.setImageResource(R.drawable.persil_jel_camasir_deterjani);

        else if (urun_adi.equals("BebelacGoldSutluBalli"))
            urun_img.setImageResource(R.drawable.bebelac_gold_sutlu_balli_irmikli);

        else if (urun_adi.equals("SleepyNaturalCocukBezi"))
            urun_img.setImageResource(R.drawable.sleepy_natural_maxi);
